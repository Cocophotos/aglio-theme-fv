...
(function() {
  var ROOT, benchmark, cache, compileTemplate, crypto, decorate, errMsg, fs, getCached, getCss, getTemplate, highlight, hljs, jade, less, markdownIt, modifyUriTemplate, moment, path, querystring, renderExample, renderSchema, schema_rendering, sha1, slug;
  ...
  schema_rendering = function(schema, flattened_struct, required) {
    var key, key2, param, properties, value, value2;
    if (flattened_struct == null) {
      flattened_struct = [];
    }
    if (required == null) {
      required = [];
    ...
    for (key in properties) {
      ...
      for (key2 in value) {
        ...
        if (key2 === "properties") {
          param['subparams'] = [];
          schema_rendering(JSON.stringify({
            'properties': value2
